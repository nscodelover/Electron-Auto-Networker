...
var tags = ['Option 1', 'Option 2', 'Option 3', 'Option 4', 'Option 5', 'Option 6', 'Option 7', 'Option 8'];
var automations = ['automation 1', 'automation 2', 'automation 3', 'automation 4', 'automation 5', 'automation 6', 'automation 7', 'automation 8', 'automation 9'];
...
var calQueueExeDate = function(delay_time, start_time) {
    if (delay_time.indexOf('Minute') > 0) {
        start_time.setMinutes(start_time.getMinutes() + parseInt(delay_time.charAt(0)));
    }
    ...
    if (delay_time.indexOf('Week') > 0) {
        start_time.setDate(start_time.getDate() + parseInt(delay_time.charAt(0)) * 7);
    }
        ...
        for (var i = 0; i < peopleAutomations.length; i++) {
            ...
            for (var j = 0; j < result.rows.length; j++) {
                if (result.rows[j].doc.queue_people_id == peopleId) {
                    if (result.rows[j].doc.queue_automation_id == peopleAutomations[i]) {
            ...
            if (actionsExistBit) {
                ...
                var docs = {};
                var automationDataTmp = getAutomationDataFromJson(peopleAutomations[i]);
                var tmpDate2 = new Date();
                var tmpDate1;
                for (var j = 0; j < automationDataTmp.automation_triggers.length; j++) {
                    for (var k = 0; k < automationDataTmp.automation_triggers[j].trigger_action.length; k++) {
                        docs._id = new Date().toJSON() + '-' + peopleAutomations[i] + '-' + automationDataTmp.automation_triggers[j]._id + '-' + automationDataTmp.automation_triggers[j].trigger_action[k]._id;
                        ...
                        docs.queue_task_name = getAutomationNameFromJson(peopleAutomations[i]);
                        docs.queue_action_state = '1';
                        tmpDate1 = calQueueExeDate(automationDataTmp.automation_triggers[j].trigger_action[k].action_delay_time, tmpDate2);
...
var drawPeopleTable = function(peopleData, pageNum) {
    var firstFilter = document.getElementById('firstFilter').checked ? '1st' : '';
    var secondFilter = document.getElementById('secondFilter').checked ? '2nd' : '';
    var thirdFilter = document.getElementById('thirdFilter').checked ? '3rd+' : '';
    ...
    peopleTableContent = '<table class="table largeTable"><thead><tr style="background-color:#f7f7f7"><th><label class="custom-checkbox-item pull-left"><input class="custom-checkbox" type="checkbox" id="bulkCheck" onClick="selectAll(this)" ' + bulkCheckEnable + '><span class="custom-checkbox-mark" style="border:1px solid black;background-color:white;"></span>';
    peopleTableContent += '</label></th><th style="text-align:center;"><i class="fa fa-user"></i></th><th>Name/Title</th><th>Company</th><th>Location</th><th>Common</th><th>Tags</th><th>Connection Values</th><th>Automation</th><th>Black List</th></tr></thead><tbody>';
        ...
        if (
            ...
            if (blFilter == true) {
                ...
                if ((itemNum >= firstItemNum) && (itemNum <= secondItemNum)) {
                    ...
                    peopleTableContent += '<tr><td style="text-align:center;"><label class="custom-checkbox-item pull-left"><input class="custom-checkbox" type="checkbox" name="individual" id="' + peopleList[i].doc._id + '" onClick="selectIndividual(this)"><span class="custom-checkbox-mark" style="border:1px solid black;background-color:white;"></span>';
                    peopleTableContent += '</label></td><td style="text-align:center;"><img src="assets/img/avatar.jpg"></td><td><div class="cellStyleName"><a href="#">' + peopleList[i].doc.people_name_first + ' ' + peopleList[i].doc.people_name_middle + ' ' + peopleList[i].doc.people_name_last + '</a></div><div class="cellStyleName1">' + peopleList[i].doc.people_title + '</div></td><td>' + peopleList[i].doc.people_company.people_company_name;
                    peopleTableContent += '</td><td>' + peopleList[i].doc.people_location + '</td><td>' + peopleList[i].doc.people_common + '</td><td><div class="controls"><select class="tagsSelect" onchange="tagsChange(this,\'' + peopleList[i].doc._id + '\')" id="tags" multiple style="min-width:300px;">';
                    for (var j = 0; j < peopleList[i].doc.people_tags.p_tag.length; j++) {
                        peopleTableContent += '<option selected>' + peopleList[i].doc.people_tags.p_tag[j] + '</option>';
                    }
                    peopleTableContent += '</select></div></td><td>' + peopleList[i].doc.people_connection + '</td><td><div class="controls"><select class="automationsSelect" onchange="automationsChange(this,\'' + peopleList[i].doc._id + '\')"  id="automations" multiple style="min-width:300px;">';
                    for (var j = 0; j < automationListValTmp.length; j++) {
                        var compareSeletTmp = false;
                        for (var k = 0; k < peopleList[i].doc.people_automations.p_automation_name.length; k++) {
                            if (automationListValTmp[j] == peopleList[i].doc.people_automations.p_automation_name[k]) {
                                peopleTableContent += '<option selected value="' + automationListValTmp[j] + '">' + automationId_nameList[automationListValTmp[j] + ''] + '</option>';
                                compareSeletTmp = true;
                                break;
                            }
                        }
                        if (!compareSeletTmp) {
                            peopleTableContent += '<option value="' + automationListValTmp[j] + '">' + automationId_nameList[automationListValTmp[j] + ''] + '</option>';
                    ...
                    peopleTableContent += '<td><span class="label label-danger" style="display:' + blacklistTmp + '">Registered</span></td>';
                ...
                if (peopleList[i].doc.people_blacklist == false) {
                    ...
                    if ((itemNum >= firstItemNum) && (itemNum <= secondItemNum)) {
                        ...
                        for (var j = 0; j < automationListValTmp.length; j++) {
                            var compareSeletTmp = false;
                            for (var k = 0; k < peopleList[i].doc.people_automations.p_automation_name.length; k++) {
                                if (automationListValTmp[k] == peopleList[i].doc.people_automations.p_automation_name[j]) {
                                    peopleTableContent += '<option selected value="' + automationListValTmp[j] + '">' + automationId_nameList[automationListValTmp[j] + ''] + '</option>';
                                    compareSeletTmp = true;
                                    break;
                                }
    ...
    var resultRes = 'Filtered ' + secondFilteredNum + ' people from ' + allPeopleNum + ' total people';
...
var searchFieldValule = function(peopleListtmp, fieldNametmp, keyValuetmp) {
    var res = [];
    var listFields = ['title', 'name', 'company', 'location', 'common', 'tags', 'connection values', 'automation'];
    for (var i = 0; i < peopleListtmp.length; i++) {
        switch (fieldNametmp) {
            case 0:
                if (peopleListtmp[i].doc.people_title.toLowerCase().indexOf(keyValuetmp) >= 0) {
                    res.push(peopleListtmp[i]);
                }
                break;
            case 1:
                if (peopleListtmp[i].doc.people_full_name.toLowerCase().indexOf(keyValuetmp) >= 0) {
                    res.push(peopleListtmp[i]);
                }
                break;
            case 2:
                if (peopleListtmp[i].doc.people_company.toLowerCase().indexOf(keyValuetmp) >= 0) {
                    res.push(peopleListtmp[i]);
                }
                break;
            case 3:
                if (peopleListtmp[i].doc.people_location.toLowerCase().indexOf(keyValuetmp) >= 0) {
                    res.push(peopleListtmp[i]);
                }
                break;
            case 4:
                if (peopleListtmp[i].doc.people_common.toLowerCase().indexOf(keyValuetmp) >= 0) {
                    res.push(peopleListtmp[i]);
                }
                break;
            case 5:
                if (searchArray(peopleListtmp[i].doc.people_tags.p_tag, keyValuetmp)) {
                    res.push(peopleListtmp[i]);
                }
                break;
            case 6:
                if (peopleListtmp[i].doc.people_connection.toLowerCase().indexOf(keyValuetmp) >= 0) {
                    res.push(peopleListtmp[i]);
                }
            case 7:
                if (searchArray(peopleListtmp[i].doc.people_automations.p_automation_name, keyValuetmp)) {
                    res.push(peopleListtmp[i]);
                }
        }
    }
...
var searchPeople = function(query, jsonRes) {
    ...
    if (
        ...
        if ((query.length - 1) == query.lastIndexOf(')') || (query.lastIndexOf('(') == 0)) {
            ...
            for (var i = 0; i < (queryArray.length - 1); i++) {
                if (
                    ...
                    var fieldName = queryArray[i].slice(queryArray[i].indexOf('(') + 1, queryArray[i].indexOf(':'));
                    var keyValue = queryArray[i].slice(queryArray[i].indexOf(':') + 1, queryArray[i].length + 1);
                    if (listFields.indexOf(fieldName) >= 0) {
                        var fieldSearchRes = searchFieldValule(peopleList, listFields.indexOf(fieldName), keyValue);
        ...
        for (var i = 0; i < peopleList.length; i++) {
            if (
                ...
                (peopleList[i].doc.people_full_name.toLowerCase().indexOf(query) >= 0) ||
                (peopleList[i].doc.people_company.people_company_name.toLowerCase().indexOf(query) >= 0) ||
                (peopleList[i].doc.people_location.toLowerCase().indexOf(query) >= 0) ||
                (peopleList[i].doc.people_common.toLowerCase().indexOf(query) >= 0) ||
                (peopleList[i].doc.people_connection.toLowerCase().indexOf(query) >= 0) ||
                searchArray(peopleList[i].doc.people_tags.p_tag, query) ||
                searchArray(peopleList[i].doc.people_automations.p_automation_name, query)
            ) {
    ...
    if (filterNum == allNum) {
        ...
        resultRes = 'Filtered ' + filterNum + ' people from ' + allNum + ' total people';
        resultResContent.innerText = resultRes;
    }
...
var selectIndividual = function(checkBox) {
    if ((checkBox.checked == false) && (document.getElementById('bulkCheck').checked == true)) {
...
var addAutomationsToSelectedPeople = function() {
    var selectedAutomations = getArrayFromSelect(document.getElementById('automationsInput'));
    if (selectedAutomations.length > 0) {
        //
        for (var i = 0; i < selectedPeopleId.length; i++) {
            table_people.get(selectedPeopleId[i]).then(function(doc) {
                var automationsTmp = doc.people_automations.p_automation_name.concat(selectedAutomations).unique();
                return table_people.upsert(doc._id, function(doc_) {
...
bulkActionSelect.addEventListener('change', function() {
    ...
    if (bulkActionSelect.value == 'automations') {
        ...
        for (var i = 0; i < automationListValTmp.length; i++) {
            automationsInputDivContent += '<option value="' + automationListValTmp[i] + '">' + automationId_nameList[automationListValTmp[i] + ''] + '</option>';
        }
...
//bulk action part
document.getElementById('automationsBulkAction').addEventListener('click', function() {
    addAutomationsToSelectedPeople();
});

document.getElementById('tagsBulkAction').addEventListener('click', function() {
    addTagsToSelectedPeople();
});

document.getElementById('blacklistBulkAction').addEventListener('click', function() {
    addBlackListToSelectedPeople();
});

document.getElementById('tagsBulkCancel').addEventListener('click', function() {
    $('#modal-input-tags').modal('hide');
    bulkActionSelect.selectedIndex = '0';
});

document.getElementById('automationsBulkCancel').addEventListener('click', function() {
    $('#modal-input-automations').modal('hide');
    bulkActionSelect.selectedIndex = '0';
});

document.getElementById('blacklistBulkCancel').addEventListener('click', function() {
    $('#modal-input-blacklist').modal('hide');
    bulkActionSelect.selectedIndex = '0';
});
...
document.getElementById('automationsBulkClose').addEventListener('click', function() {
    $('#modal-input-automations').modal('hide');
    bulkActionSelect.selectedIndex = '0';
});

document.getElementById('blacklistBulkClose').addEventListener('click', function() {
    $('#modal-input-blacklist').modal('hide');
    bulkActionSelect.selectedIndex = '0';
});
