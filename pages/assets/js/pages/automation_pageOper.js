...
var deleteActionModalTriggerId = document.getElementById('deleteActionModalTriggerId');
var deleteActionModalActionId = document.getElementById('deleteActionModalActionId');

var updateActionModalTriggerId = document.getElementById('updateActionModalTriggerId');
var updateActionModalActionId = document.getElementById('updateActionModalActionId');

//changable doms in modal window
var acceptConnectionRequestSelect = document.getElementById('acceptConnectionRequestSelect');
...
var sendConnectionRequestSelect = document.getElementById('sendConnectionRequestSelect');
...
var getActionParameter = function(actionStyle) {
    ...
    switch (parseInt(actionStyle)) {
        ...
        case 5:
            ...
            break;
        case 6:
            returnVal = document.getElementById('transferCampaignSelect').options[document.getElementById('transferCampaignSelect').selectedIndex].value;
            break;
        case 7:
            returnVal = document.getElementById('acceptConnectionRequestSelect').options[document.getElementById('acceptConnectionRequestSelect').selectedIndex].value;
    }
    return returnVal;
};
...
var setActionParameter = function(actionData) {
    switch (parseInt(actionData.action_style)) {
        case 1:
            document.getElementById('visitProfileCheck').checked = actionData.action_parameter;
            break;
        case 2:
            var selTmp = document.getElementById('sendMessageSelect');
            ...
            if (actionData.action_parameter.event == 'customMessages') {
                ...
                document.getElementById(selTmp.id + 'CM').value = actionData.action_parameter.content;
            ...
            if (actionData.action_parameter.event == 'selectFromTemplate') {
                ...
                for (var i = 0; i < document.getElementById(selTmp.id + 'List').options.length; i++) {
                    if (actionData.action_parameter.content == document.getElementById(selTmp.id + 'List').options[i].value) {
                        document.getElementById(selTmp.id + 'List').options[i].selected = true;
                    }
                }
        ...
        case 5:
            ...
            break;
        case 6:
            for (var i = 0; i < document.getElementById('transferCampaignSelect').options.length; i++) {
                if (document.getElementById('transferCampaignSelect').options[i].value == actionData.action_parameter) {
                    document.getElementById('transferCampaignSelect').options[i].selected = true;
                }
            }
            break;
        case 7:
            for (var i = 0; i < document.getElementById('acceptConnectionRequestSelect').options.length; i++) {
                if (document.getElementById('acceptConnectionRequestSelect').options[i].value == actionData.action_parameter) {
                    document.getElementById('acceptConnectionRequestSelect').options[i].selected = true;
                }
            }
    }
};
...
var setActionModal = function(triggerId, actionId) {
    ...
    for (var i = 0; i < response.automation_triggers.length; i++) {
        if (response.automation_triggers[i]._id == triggerId) {
            for (var j = 0; j < response.automation_triggers[i].trigger_action.length; j++) {
                if (response.automation_triggers[i].trigger_action[j]._id == actionId) {
                    //set action edit modal window
                    for (var k = 0; k < addActionSelect.options.length; k++) {
                        if (addActionSelect.options[k].value == response.automation_triggers[i].trigger_action[j].action_style) {
                    ...
                    for (var k = 0; k < conditionSelect.options.length; k++) {
                        if (response.automation_triggers[i].trigger_style == conditionSelect.options[k].value) {
                            conditionSelect.selectedIndex = k;
                        }
                    }
                    for (var k = 0; k < delayTimeSelect.options.length; k++) {
                        if (response.automation_triggers[i].trigger_action[j].action_delay_time.indexOf(delayTimeSelect.options[k].value) > 0) {
                            delayTimeSelect.selectedIndex = k;
                        }
                        delayTimeNum.value = response.automation_triggers[i].trigger_action[j].action_delay_time.charAt(0);
                    }
                    ////set Action Parameter function part.
                    setActionParameter(response.automation_triggers[i].trigger_action[j]);
...
triggerSaveBtn.addEventListener('click', function() {
    if (!delayTimeNum.value) {
        alert('Please insert delay time');
        return false;
    }
    ...
    var delayTime = '';
    delayTime = delayTimeNum.value + ' ' + delayTimeSelect.options[delayTimeSelect.selectedIndex].value;
    ...
    if (!triggerJsonToDraw.automation_triggers) {
        ...
        if (actionEditBit == true) {
            ...
            actionTmp.action_parameter = getActionParameter(addActionSelect.options[addActionSelect.selectedIndex].value);
        ...
        for (var i = 0; i < triggerJsonToDraw.automation_triggers.length; i++) {
            if (triggerJsonToDraw.automation_triggers[i].trigger_style == conditionSelect.options[conditionSelect.selectedIndex].value) {
                if (actionEditBit == true) {
                    for (var j = 0; j < triggerJsonToDraw.automation_triggers[i].trigger_action.length; j++) {
                        if (triggerJsonToDraw.automation_triggers[i].trigger_action[j]._id == updateActionModalActionId.value) {
                            actionTmp.action_queue_number = triggerJsonToDraw.automation_triggers[i].trigger_action[j].action_queue_number;
                            actionTmp.action_parameter = getActionParameter(addActionSelect.options[addActionSelect.selectedIndex].value);
                            triggerJsonToDraw.automation_triggers[i].trigger_action.splice(j, 1);
                            triggerJsonToDraw.automation_triggers[i].trigger_action.push(actionTmp);
                        }
                    }
                } else {
                    actionTmp.action_queue_number = triggerJsonToDraw.automation_triggers[i].trigger_action.length + 1;
                    actionTmp.action_parameter = getActionParameter(addActionSelect.options[addActionSelect.selectedIndex].value);
                    triggerJsonToDraw.automation_triggers[i].trigger_action.push(actionTmp);
                }
...
addActionSelect.addEventListener('change', function() {
    ...
    if (selectVal != "none") {
        tagContent.innerHTML = document.getElementById(selectVal).innerHTML;
        if ((selectVal == 'sendRecomRequest') || (selectVal == 'sendConnectionRequest') || (selectVal == 'sendMessage')) {
            document.getElementById(selectVal + 'Select').selectedIndex = 0;
            document.getElementById(selectVal + 'SelectTemplate').style.display = 'block';
            document.getElementById(selectVal + 'SelectCustom').style.display = 'none';
        }
        if (selectVal == "addTag") {
            $('#addTagSelect').select2({
                tags: true,
...
deleteActionBtn.addEventListener('click', function() {
    ...
    if (response.automation_triggers[triggerNum].trigger_action.length == 1) {
        ...
        for (var j = 0; j < response.automation_triggers[triggerNum].trigger_action.length; j++) {
            if (parseInt(response.automation_triggers[triggerNum].trigger_action[j].action_queue_number) > parseInt(queueNum)) {
                response.automation_triggers[triggerNum].trigger_action[j].action_queue_number = parseInt(response.automation_triggers[triggerNum].trigger_action[j].action_queue_number) - 1;
            }
        }
        response.automation_triggers[triggerNum].trigger_action.splice(actionNum, 1);
    }
...
var changeQueueDown = function(triggerId, actionId, action_queue_number) {
    for (var i = 0; i < triggerJsonToDraw.automation_triggers.length; i++) {
        if (triggerJsonToDraw.automation_triggers[i]._id == triggerId) {
            for (var j = 0; j < triggerJsonToDraw.automation_triggers[i].trigger_action.length; j++) {
                if (triggerJsonToDraw.automation_triggers[i].trigger_action[j].action_queue_number == (action_queue_number + '')) {
                    //set action edit modal window
                    triggerJsonToDraw.automation_triggers[i].trigger_action[j].action_queue_number = parseInt(action_queue_number) + 1;
                } else if (triggerJsonToDraw.automation_triggers[i].trigger_action[j].action_queue_number == (parseInt(action_queue_number) + 1 + '')) {
                    triggerJsonToDraw.automation_triggers[i].trigger_action[j].action_queue_number = parseInt(action_queue_number);
                }
            }
        }
    }
...
var drawAutomationTable = function(automationData) {
    ...
    for (var i = 0; i < automationData.total_rows; i++) {
        firstAutomationTableContent += '<tr>' +
            ...
            '<td>' + automationDataRows[i].doc.automation_no_people_active + '</td>' +
            '<td>' + automationDataRows[i].doc.automation_engagement + '%' + '</td>' +
            '<td>' + automationDataRows[i].doc.automation_connection + '</td>' +
            '<td>' + automationDataRows[i].doc.automation_clicks + '</td>' +
            '<td>' +
            '<button type="button" class="btn btn-info" onclick="openEditPage(\'' + automationDataRows[i].doc._id + '\')"><i class="fa fa-edit"></i></button>' +
            '<button type="button" class="btn btn-danger" data-toggle="modal" data-dismiss="modal" data-target="#modal-delete-automation" onclick="deleteAutomationInfo(\'' + automationDataRows[i].doc._id + '\')"><i class="fa fa-trash-o"></i></button>' +
            '</td>' +
            '</tr>';
    }
...
var drawTriggerTableByJson = function(docInfo) {
    ...
    for (var i = 0; i < docInfo.automation_triggers.length; i++) {
        triggerTmp += '<table class="table largeTable">' +
            '<thead>' +
            '<tr>' +
            '<th>' + selectTriggerStyle(docInfo.automation_triggers[i].trigger_style) + '</th>' +
        ...
        for (var k = 0; k < trigger_action.length; k++) { //loop for queue number
            for (var j = 0; j < trigger_action.length; j++) {
                if (k == (parseInt(trigger_action[j].action_queue_number) - 1)) {
                    ...
                    triggerTmp += '<tr>' + '<input type="hidden" value ="' + trigger_action[j]._id + '">' +
                        '<td>' + selectActionStyle(trigger_action[j].action_style) + '</td>' +
                        '<td>' + trigger_action[j].action_delay_time + '</td>' +
                        '<td>' +
                        '<button type="button" class="btn btn-info" data-toggle="modal" data-dismiss="modal"  onclick="setActionModal(\'' + docInfo.automation_triggers[i]._id + '\',\'' + trigger_action[j]._id + '\')"><i class="fa fa-edit"></i></button>' +
                        '<button type="button" class="btn btn-danger" data-toggle="modal" data-dismiss="modal" data-target="#modal-delete-action" onclick="deleteActionModal(\'' + docInfo.automation_triggers[i]._id + '\',\'' + trigger_action[j]._id + '\')"><i class="fa fa-trash-o"></i></button>' +
                        '</td><td>';
                    if (k == 0) {
                        ...
                        if (k == (trigger_action.length - 1)) {
                            ...
                            triggerTmp += '<button type="button" class="btn btn-default" onclick="changeQueueDown(\'' + docInfo.automation_triggers[i]._id + '\',\'' + trigger_action[j]._id + '\',\'' + trigger_action[j].action_queue_number + '\')"><i class="fa fa-arrow-down"></i></button>';
                        }
                    } else {
                        triggerTmp += '<button type="button" class="btn btn-default" onclick="changeQueueUp(\'' + docInfo.automation_triggers[i]._id + '\',\'' + trigger_action[j]._id + '\',\'' + trigger_action[j].action_queue_number + '\')"><i class="fa fa-arrow-up"></i></button>';
                        if (k == (trigger_action.length - 1)) {
                            triggerTmp += '<button type="button" class="btn btn-default" disabled><i class="fa fa-arrow-down"></i></button>';
                        } else {
                            triggerTmp += '<button type="button" class="btn btn-default" onclick="changeQueueDown(\'' + docInfo.automation_triggers[i]._id + '\',\'' + trigger_action[j]._id + '\',\'' + trigger_action[j].action_queue_number + '\')"><i class="fa fa-arrow-down"></i></button>';
...
getAllTableMessageTemplate().then(function(response) {
    for (var i = 0; i < response.rows.length; i++) {
        ...
        opt.text = response.rows[i].doc.message_text;
